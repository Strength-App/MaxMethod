...
- **`window.AudioContext` / `window.webkitAudioContext`** — single consumer `ToolsContext.jsx` (timer-end beep); the mock covers exactly the AudioContext surfaces the consumer touches; per-test opt-out documented inline for the "audio unavailable" branch.
