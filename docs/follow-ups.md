...
- **Resolved.** Batch 8, commit `cea75a9`, PR [#90](https://github.com/Strength-App/MaxMethod/pull/90) (pending merge), 2026-06-05.
...
- **Resolved.** Batch 8, commit `5188dfa`, PR [#90](https://github.com/Strength-App/MaxMethod/pull/90) (pending merge), 2026-06-05.
...
- **Resolved.** Batch 8 (documented), PR [#90](https://github.com/Strength-App/MaxMethod/pull/90) (pending merge), 2026-06-05.
...
- **Resolved.** Batch 11, commit `16dc159`, PR [#94](https://github.com/Strength-App/MaxMethod/pull/94), 2026-06-06.
- **How.** Chose **Option A (local overlay)**. `reviewProgram.jsx` now imports the canonical `MOVEMENT_PATTERNS` from `config/exercises.js` as `CANONICAL_MOVEMENT_PATTERNS` and rebuilds only the `'Squat Pattern'` key as `[...CANONICAL_MOVEMENT_PATTERNS['Squat Pattern'], 'Squats', 'Back Squat']`; `EXERCISE_EQUIPMENT` is consumed directly (the canonical map already carries the `'Squats'`/`'Back Squat'` keys). Options B (import the peer page's alias map) and C (relocate `EXERCISE_NAME_ALIASES` into config) were rejected — B re-introduces page-to-page coupling, C edits Batch 12's `exerciseLibrary.jsx` and breaks per-batch isolation. The two aliases now append to the end of the swap list rather than sitting mid-list; membership is unchanged and the characterization test (`reviewProgram.test.jsx`) asserts membership + count, not order, so the reorder is an accepted cosmetic change. Full decision at [`docs/decisions.md#reviewprogram-squat-alias-overlay`](decisions.md#reviewprogram-squat-alias-overlay).
- **Original entry (preserved for context).**
...
- **Resolved.** Batch 12, PR pending, 2026-06-06.
- **How.** Batch 12 relocated `ALL_EXERCISES`/`ALL_EXERCISE_NAMES` out of `pages/exerciseLibrary.jsx` into `config/exercises.js` (see [`docs/decisions.md#exercise-data-layer-relocation-and-alias-home`](decisions.md#exercise-data-layer-relocation-and-alias-home)), removing the upside-down `utils/ → pages/` import that had blocked this work. With the name list now in config, `utils/customExercises.js` gained `getAllExerciseNames` (`() => [...ALL_EXERCISE_NAMES, ...getCustomExerciseNames()]`) and `isValidExercise` (case-insensitive membership), each with plain-language JSDoc and tests against the real catalog plus a session-added custom name. `exerciseLibrary`'s own two inline `JSON.parse(localStorage.getItem('customExercises'))` reads (the `CustomExercisesSection` initializer and the focusExercise deep-link lookup) now consume `getCustomExerciseNames`. **Remaining (out of this entry's scope):** `customDay`, `history`, and `logger` still carry byte-identical inline copies of these helpers; they migrate onto the shared util in their own batches (customDay Batch 13, logger Batch 14; history is a small opportunistic cleanup whenever that file is next touched).
- **Original entry (preserved for context).**
...
- **Resolved.** Batch 14, PR pending, 2026-06-06.
- **How.** Chose **Option 1 (shared util)**. Added `src/utils/restDuration.js` exporting `getRestSeconds(name)`, with `restDuration.test.js` pinning the 120/90 mapping (big-three → 120s, else 90s; case-insensitive; missing name → 90s). Both `pages/logger.jsx` and `pages/day.jsx` now import it and their inline `getRestSeconds` + `BIG_THREE` copies are deleted — the bodies were byte-identical modulo the parameter name, so behavior is unchanged. Per the entry's warning, `BIG_THREE` is kept **module-private** to `restDuration.js` (not exported) so the loose-substring rest-duration big-three can't be mistaken for the stricter PR-detection notion; no consumer needed it externally. The full dedup (both pages in one batch, day.jsx touched ahead of its Batch 15 only for this verbatim swap) was an explicit scope decision confirmed with the user at batch start.
- **Original entry (preserved for context).**
