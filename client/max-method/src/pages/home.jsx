...
function Home() {
  ...
  // Hoisted: `?. === 'custom'` evaluates to false for null/loading/error
  // states, so the badge's `!isCustom` guard renders the badge consistently
  // when there's no program yet, while still hiding it for custom workouts.
  const isCustom = displayWorkout?.type === 'custom';

  // Derived values only meaningful when displayWorkout exists. Pre-initialized
  // so the normal-render branch can reference them unconditionally.
  let programTitle = '';
  let totalDays = 0;
  let completedDays = 0;
  if (displayWorkout) {
  ...
  // Single render tree. Badge sits above the loading / error / no-program /
  // normal branches so any logged-in user sees a consistent leveling surface
  // — null-state users see the empty-state prompt, non-null users see their
  // badge. The `!isCustom` guard preserves today's behavior of hiding the
  // badge for custom workouts.
  return (
    <div className="home-page-container">
      ...
      {loading ? (
        <p className="status-msg" role="status" aria-live="polite">Loading your program…</p>
      ) : error ? (
        <p className="status-msg status-msg--error" role="alert">Error loading workout: {error}</p>
      ) : !displayWorkout ? (
